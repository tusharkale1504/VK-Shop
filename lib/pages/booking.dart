...
class _BookingState extends State<Booking> {
      ...
      if (picked.hour >= 9 && picked.hour <= 22) {
        if (!_appointments.contains(picked)) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Appointment time selected: ${picked.format(context)}')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('There is already an appointment at this time.')),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Appointments can only be scheduled between 9:00 and 22:00.')),
        );
      }
    }
  }
  ...
  Future<void> _checkAndBookAppointment(BuildContext context) async {
    ...
    if (alreadyBooked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('You already have an appointment for this service on this date.'),
        ),
      );
    } else {
